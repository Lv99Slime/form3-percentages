  ...
  const stepper = (h, vb, steps, opt = {}) => {
    ...
    h.innerHTML = `<div style="width:100%;text-align:center">
      ...
      <div class="ictrl">
        ...
        <label>${window.UI.t("step")} <span class="ival stepv">1</span> / ${N}　<span class="step-hint">${window.UI.t("drag")}</span></label>
        <input class="steps-r" aria-label="${window.UI.t('step')}" type="range" min="0" max="${N}" step="0.01" value="1">
      </div></div>`;

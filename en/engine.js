...
(function () {
  ...
  function fitSlide() {
    if (window.innerWidth <= 1080) {
      slideEl.querySelectorAll('.col-fit, .visual-host').forEach(el => el.style.transform = 'none');
      return;
    }
  ...
  function ensureZoom() {
    ...
    bar.innerHTML = `<div class="zoom-badge" id="zoomBadge"></div>
      <button class="zoom-btn" id="zoomSol">${U.t('show')}</button>
      <button class="zoom-btn zoom-x" id="zoomClose">${U.t('close')}</button>`;
    ...
    bar.querySelector('#zoomSol').onclick = () => {
      ...
      bar.querySelector('#zoomSol').textContent = box.classList.contains('show') ? U.t('hide') : U.t('show');
      typeset(box);
    };
  }
  function showZoom(s, badgeTail) {
    ensureZoom();
    document.getElementById('zoomBadge').innerHTML = `${U.chapter(s.ch)} · ${s.sec} ${s.secName || ''}　${badgeTail}`;
  ...
  function openExampleModal(s) {
    ensureZoom();
    document.getElementById('zoomSol').style.display = s.example.ans || s.example.steps?.length ? '' : 'none';
    document.getElementById('zoomSol').textContent = U.t('show');
    document.getElementById('zoomBody').innerHTML =
      `<div class="zoom-q">${s.example.q}</div>
       <div class="zoom-sol" id="zoomSolBox">
         ...
         ${s.example.ans ? `<div class="zoom-ans">${U.t('answer')}: ${s.example.ans}</div>` : ''}
       </div>`;
    showZoom(s, s.example.label || U.t('worked'));
  ...
  (function buildCover() {
    ...
    DECK.forEach(c => {
      ...
      card.innerHTML = `<div class="cc-num">${U.chapter(c.ch)}</div>
  ...
  function render() {
    const s = flat[idx];
    if (!s) return;
    if (window.MathJax?.typesetClear) MathJax.typesetClear([slideEl]);
    ...
    if (s.type === 'divider') {
      ...
      slideEl.innerHTML = `
        <div>
          <div class="dv-num">${U.chapter(s.ch)}</div>
          <div class="dv-title">${s.title}</div>
          <div class="dv-list">${s.sections.map(x => `<span class="dv-chip">${x}</span>`).join('')}</div>
        </div>`;
      crumbEl.innerHTML = `${U.chapter(s.ch)}　<b>${s.title}</b>`;
    } else {
      slideEl.className = 'slide';
      // 左：概念欄
      const info = document.createElement('div');
      info.className = 'slide-info';
      let html = `<div class="badge">${U.chapter(s.ch)} · ${s.sec} ${s.secName || ''}</div>
      ...
      if (s.example) {
        html += `<div class="example">
          <div class="ex-head">${s.example.label || U.t('worked')}<button class="ex-zoom" title="${U.t('enlarge')}">${U.t('enlarge')}</button></div>
          <div class="ex-q">${s.example.q}</div>
          <button class="ex-toggle" ${s.example.ans || s.example.steps?.length ? "" : "hidden"}>${U.t('show')}</button>
          <div class="ex-sol">
            ${s.example.steps ? `<ol>${s.example.steps.map(t => `<li>${t}</li>`).join('')}</ol>` : ''}
            ${s.example.ans ? `<div class="ex-ans">${U.t('answer')}: ${s.example.ans}</div>` : ''}
          </div>
        </div>`;
      }
      ...
      visZoom.title = U.t('enlarge');
      visZoom.textContent = U.t('enlarge');
      ...
      if (typeof s.visual === 'function') {
        try { s.visual(host); } catch (e) { host.textContent = U.t("visualError"); console.error(e); }
      } else {
      ...
      if (tog) {
        ...
        tog.onclick = () => {
          ...
          tog.textContent = sol.classList.contains('show') ? U.t('hide') : U.t('show');
      ...
      crumbEl.innerHTML = `${U.chapter(s.ch)} · ${s.sec} <b>${s.title}</b>`;
    }
  ...
  $('homeBtn').onclick = () => { closeZoom(); setPen(false); setLaser(false); clearPen(); app.classList.add('hidden'); $('cover').classList.remove('hidden'); history.replaceState(null, '', location.pathname + location.search); };
  $('startBtn').onclick = () => { $('cover').classList.add('hidden'); app.classList.remove('hidden'); fitPen(); go(idx); };
  ...
  $('dkFull').onclick = async () => {
    try {
      if (!document.fullscreenElement) await document.documentElement.requestFullscreen();
      else await document.exitFullscreen();
    } catch { const status = $('mathStatus'); status.textContent = U.t('fullError'); status.classList.remove('hidden'); }
  };
  ...
  document.addEventListener('keydown', e => {
    if (e.key !== 'Escape' && (e.target.matches('input, textarea, select') || e.target.isContentEditable || (e.target.closest('button') && [' ', 'Enter'].includes(e.key)))) return;
